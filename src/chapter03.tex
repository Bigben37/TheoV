...
\begin{enumerate}[A)]
    ...
    \end{equation}
    $\mathcal{Z}(\beta, V, \gamma)$ ist die großkanonische (große) Zustandssumme. \\
    ...
    \begin{enumerate}[a)]
        ...
        \begin{equation}
            \begin{split}
                ...
                & \Rightarrow \frac{p_{x \nu}^2}{2 m} = \frac{n^2 h^2}{8 m a^2} \qquad \alpha := \frac{h^2}{8 m a^2 k T}
            \end{split}
        \end{equation}
...
\begin{equation}
    Z = W e^{-\frac{E}{k T}} = \left[ \frac{V}{N} \left( \frac{4 \pi m E}{3 h^2 N} \right)^{3/2} e^{5/2} \right]^N e^{-\frac{E}{k T}}
\end{equation}
Entropie:
\begin{equation}
...
\begin{enumerate}[i)]
    ...
    \begin{enumerate}[a)]
        \item Rotation um die $\chi$-Achse (Modell hier: Massenpunkte, $I_3 = 0$)
    ...
    \begin{equation}
        \Rightarrow c_v = \frac{6}{2} N k = 3 N k \qquad c_p = 4 N k
    \end{equation}
...
\begin{equation}
    \begin{split}
        ...
        E &= F + T S \Rightarrow E = E_\text{tr} + E_\text{rot} + E_\text{vib} \quad \text{mit} \quad E_\text{tr} = F_\text{tr} + T S_\text{tr}, \text{ usw.}
    \end{split}
\end{equation}
\begin{enumerate}[A)]
    \item Rotation
...
\begin{enumerate}[A)]
    ...
    $N_{i, \text{BE}} = \frac{1}{e^{(\epsilon_i - \mu) / k T}-1}$ bzw. $N_{i, \text{FD}} = \frac{1}{e^{(\epsilon_i - \mu)/kT}+1}$ sind die mittleren
...
\begin{equation}
    N = - \pdi{\Omega}{\mu}{T, V} =
    \begin{cases}
\sum_i \left[ \sigma^{-1} e^{\epsilon_i / k T} \pm 1 \right]^{-1} & \begin{pmatrix} + \text{FD} \\ - \text{BE} \end{pmatrix} \\
\sum_i \sigma e^{-\epsilon_i / k T} & (\text{MB})
    \end{cases}
\end{equation}
...
\end{figure}
    ...
    \end{equation}
    ist für Kupfer z.B. 50000$\,$K. Daher sind bereits bei Zimmertemperatur die Leitungselektronen stark entartet.
    \begin{equation}
